...
contract VotingEscrowTest is BaseTest {
    ...
    function testDepositForWithManagedNFT() public {
        ...
        assertEq(CEDA.allowance(address(escrow), address(lockedManagedReward)), 0);
        ...
        uint256 pre = CEDA.balanceOf(address(lockedManagedReward));
        ...
        emit NotifyReward(address(escrow), address(CEDA), 604800, reward);
        vm.expectEmit(false, false, false, true, address(escrow));
        emit MetadataUpdate(mTokenId);
        escrow.depositFor(mTokenId, reward);
        uint256 post = CEDA.balanceOf(address(lockedManagedReward));
    ...
    function testCreateLock() public {
        CEDA.approve(address(escrow), 1e25);
    ...
    function testWithdraw() public {
        ...
        uint256 preBalance = CEDA.balanceOf(address(owner));

        skipAndRoll(lockDuration);
        escrow.withdraw(1);

        uint256 postBalance = CEDA.balanceOf(address(owner));
    ...
    function testMergeWithFromLockTimeGreaterThanToLockTime() public {
        ...
        uint256 CEDASupply = escrow.supply();
        ...
        assertEq(escrow.supply(), CEDASupply);
    ...
    function testCannotSplitOverflow() public {
        ...
        uint256 escrowBalance = CEDA.balanceOf(address(escrow));
    ...
    function testCannotDelegateBySigWithInvalidNonce() public {
        ...
        deal(address(CEDA), alice, TOKEN_100K);
