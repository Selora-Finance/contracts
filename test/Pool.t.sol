...
contract PoolTest is BaseTest {
    ...
    function deployPoolCoins() public {
        ...
        amounts[1] = 1e25;
        amounts[2] = 1e25;
        mintToken(address(CEDA), owners, amounts);
    ...
    function increaseLock() public {
        ...
        assertEq(CEDA.balanceOf(address(escrow)), TOKEN_1);
    }
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(CEDA.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(CEDA.balanceOf(address(escrow)), 3 * TOKEN_1);
    ...
    function deployPoolFactoryGauge() public {
        ...
        CEDA.approve(address(gaugeFactory), 15 * TOKEN_100K);
    ...
    function addGaugeAndVotingRewards() public {
        ...
        CEDA.approve(address(bribeVotingReward), POOL_1);

        bribeVotingReward.notifyRewardAmount(address(CEDA), POOL_1);
    ...
    function createLock2() public {
        ...
        skip(1);
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(CEDA.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        deal(address(CEDA), address(minter), POOL_1);

        vm.startPrank(address(minter));
        CEDA.approve(address(voter), POOL_1);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.deposit(POOL_1);
        skip(1);
        uint256 before = CEDA.balanceOf(address(owner));
        skip(1);
        uint256 earned = gauge.earned(address(owner));
        gauge.getReward(address(owner));
        skip(1);
        uint256 after_ = CEDA.balanceOf(address(owner));
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pool), address(gauge), POOL_1);
        owner3.deposit(address(gauge), POOL_1);
        uint256 before = CEDA.balanceOf(address(owner3));
        skip(1);
        owner3.getGaugeReward(address(gauge), address(owner3));
        uint256 after_ = CEDA.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
