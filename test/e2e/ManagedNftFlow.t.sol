...
contract ManagedNftFlow is ExtendedBaseTest {
    ...
    function _setUp() public override {
        CEDA.approve(address(escrow), TOKEN_1);
    ...
    function testSimpleManagedNftFlow() public {
        ...
        rewards[0] = address(CEDA);
        rewards[1] = address(USDC);

        // create CEDA bribe for next epoch
        _createBribeWithAmount(bribeVotingReward, address(CEDA), TOKEN_1 * 2);
        ...
        // simulate rebases for epoch 0
        deal(address(CEDA), address(distributor), TOKEN_1);
        ...
        assertEq(escrow.supply(), supply);
        assertEq(CEDA.balanceOf(address(lockedManagedReward)), TOKEN_1);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), 0);
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), TOKEN_1 / 2);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId2), TOKEN_1 / 2);

        // epoch 1:
        // simulate rebase + non-compounded CEDA rewards
        ...
        uint256 pre = CEDA.balanceOf(address(owner4));
        vm.prank(address(voter));
        bribeVotingReward.getReward(mTokenId, rewards);
        uint256 post = CEDA.balanceOf(address(owner4));
        ...
        CEDA.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(CEDA), ((TOKEN_1 * 2) * 3) / 4);
        vm.stopPrank();
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId), 0);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId2), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId2), 0);
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), TOKEN_1);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId), ((TOKEN_1 * 2) * 3) / 4 / 2);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId2), TOKEN_1);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId2), ((TOKEN_1 * 2) * 3) / 4 / 2);
        ...
        skip(1 hours);
        pre = CEDA.balanceOf(address(escrow));
        vm.prank(address(owner2));
        voter.withdrawManaged(tokenId2);
        post = CEDA.balanceOf(address(escrow));
        ...
        assertEq(CEDA.balanceOf(address(lockedManagedReward)), TOKEN_1 * 2);
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId2), 0);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId2), ((TOKEN_1 * 2) * 3) / 4 / 2);
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), 0);
        assertEq(CEDA.balanceOf(address(lockedManagedReward)), 0);
        ...
        // create CEDA bribe for epoch 4
        _createBribeWithAmount(bribeVotingReward, address(CEDA), TOKEN_1);

        skipToNextEpoch(1);

        // test normal nft behavior post withdrawal
        // ~= approx TOKEN_1 * 3 / 4, some drift due to voting power decay
        assertEq(bribeVotingReward.earned(address(CEDA), tokenId), 749095271054930289);
    ...
    function testTransferManagedNftFlow() public {
        ...
        vm.startPrank(address(owner4));
        CEDA.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(CEDA), TOKEN_1);
        vm.stopPrank();
        ...
        assertEq(freeManagedReward.earned(address(CEDA), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId2), TOKEN_1 / 2);
        ...
        // collect rewards from bribe
        pre = CEDA.balanceOf(address(owner3));
        vm.prank(address(voter));
        bribeVotingReward.getReward(mTokenId, rewards);
        post = CEDA.balanceOf(address(owner3));
        assertEq(post - pre, TOKEN_1 * 2);

        // distribute reward to managed nft depositors
        vm.startPrank(address(owner3));
        CEDA.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(CEDA), TOKEN_1 * 2);
        vm.stopPrank();
        ...
        assertEq(freeManagedReward.earned(address(CEDA), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(CEDA), tokenId2), (TOKEN_1 * 5) / 2);
    ...
    function testManagedNftRebaseFlow() public {
        ...
        CEDA.approve(address(escrow), TOKEN_1M);
        ...
        assertGt(CEDA.balanceOf(address(distributor)), 0);
        ...
        assertEq(CEDA.balanceOf(address(lockedManagedReward)), managedRebaseTotal);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), 0);
        assertEq(escrow.supply(), supply);

        // epoch 3: claim rebases
        skipToNextEpoch(1);
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), managedRebaseTotal);
        minter.updatePeriod();
        ...
        assertEq(lockedManagedReward.earned(address(CEDA), tokenId), managedRebaseTotal - rebase);
        assertEq(escrow.supply(), supply);
        ...
        skip(1 hours);
        uint256 pre = CEDA.balanceOf(address(escrow));
        voter.withdrawManaged(tokenId);
        uint256 post = CEDA.balanceOf(address(escrow));
